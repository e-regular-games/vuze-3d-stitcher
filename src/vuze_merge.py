...
def usage():
    ...
    print('--depth <action>\t\t\tRead the color/distance information from <file_prefix>.json (same as input image) and add the coordinate data to the provided coefficients file provided by alignment. All other options will be ignored, except for the input image which is required. Actions: {add-seam-a, add-only, linreg, kabsch}')
...
def seam_analysis(action, config, coeffs, debug):
    ...
    if len(action.split('-')) != 3:
        print('Unknown action expected: add-seam-0, add-seam-1, add-seam-2, add-seam-3')
        exit(1)

    idx = int(action.split('-')[2]) * 2
    seam_images = [images[(i-2) % 8] for i in range(4)]
    seam_calibs = [calibration[(i-2) % 8].t for i in range(4)]
    seam = SeamCalibration(seam_images, seam_calibs, debug)
    seam.determine_coordinates(patches)
    ...
    if action == 'kabsch' or action == 'linreg' or action == 'homography':
        for d in depths:
            d._mode = action
...
def depth_analysis(action, config, coeffs, debug):
    ...
    for ii in range(0, 8, 2):
        depth_cal = DepthCalibration(images[ii], images[ii+1], \
                                     calibration[ii].t, calibration[ii+1].t, \
                                     debug.set_subplot(1, 4, int(ii / 2)))
